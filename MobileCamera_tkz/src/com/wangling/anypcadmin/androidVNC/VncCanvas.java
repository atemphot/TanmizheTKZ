...
public class VncCanvas extends ImageView {
	...
	public boolean is_viewonly = true;
	...
	void initializeVncCanvas(ConnectionBean bean, final Runnable setModes) {
		...
		final ProgressDialog pd = ProgressDialog.show(getContext(), 
				getResources().getString(R.string.msg_vnc_connecting), 
				getResources().getString(R.string.msg_protocol_handshake), 
			...
			public void onCancel(DialogInterface dialog) {
				...
				handler.post(new Runnable() {
					public void run() {
						Utils.showErrorMessage(getContext(), getResources().getString(R.string.msg_vnc_connection_aborted));
					}
				});
			}
		});
		...
		Thread t = new Thread() {
			public void run() {
				try {
					...
					handler.post(new Runnable() {
						public void run() {
							pd.setMessage(getResources().getString(R.string.msg_download_first_frame));
						}
					});
	...
	void onServerScaleSize(int dx, int dy, int cx, int cy) throws IOException {
		
		Log.i(TAG, "Server scale size is " + cx + " x " + cy);

		if (bitmapData != null) bitmapData.dispose();
		
		rfb.setFramebufferSize(cx, cy);
	...
	final Runnable auto_refresh_runnable = new Runnable() {
		public void run() {
			try {
				bitmapData.writeFullUpdateRequest(false);
			} catch (Exception e) {
		...
		try {
			...
			while (maintainConnection) {
				...
				case RfbProto.FramebufferUpdate:
					...
					handler.removeCallbacks(auto_refresh_runnable);
					...
					//setEncodings(true);   Move up!!!
					bitmapData.writeFullUpdateRequest(!fullUpdateNeeded);
					
					handler.postDelayed(auto_refresh_runnable, 5000);
				...
				case RfbProto.ResizeFrameBuffer:
					byte pad = rfb.is.readByte();
					int cx = rfb.is.readUnsignedShort();
					int cy = rfb.is.readUnsignedShort();
					final Display display = pd.getWindow().getWindowManager().getDefaultDisplay();
					onServerScaleSize(display.getWidth(), display.getHeight(), cx, cy);
					handler.post(setModes);
					break;
